...
class TarjetaRojaListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TarjetaRoja
        fields = ['id', 'code', 'title', 'description', 'status', 'priority', 
                 'category', 'work_area', 'created_by', 'assigned_to', 'approved_by',
...
class TarjetaRojaDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TarjetaRoja
        fields = ['id', 'code', 'title', 'description', 'category', 'work_area',
                 'status', 'priority', 'sector', 'motivo', 'destino_final',
...
class TarjetaRojaCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TarjetaRoja
        fields = ['title', 'description', 'category_id', 'work_area_id', 'priority',
                 'sector', 'motivo', 'destino_final', 'assigned_to_id',
    ...
    def create(self, validated_data):
        ...
        if work_area_id:
            try:
                work_area = WorkArea.objects.get(id=work_area_id, category=category)
...
class TarjetaRojaUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TarjetaRoja
        fields = ['title', 'description', 'priority', 'sector', 'motivo', 
                 'destino_final', 'assigned_to_id', 'estimated_resolution_date',
                 'status', 'resolution_notes', 'actual_resolution_date']
    ...
    def update(self, instance, validated_data):
        ...
        if 'status' in validated_data:
            ...
            elif new_status == 'resolved' and 'actual_resolution_date' not in validated_data:
                from django.utils import timezone
                validated_data['actual_resolution_date'] = timezone.now().date()
